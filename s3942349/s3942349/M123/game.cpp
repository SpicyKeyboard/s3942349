...
bool Game::loadBoard() {
    board->load(0); //displays an empty board
    ...
    while(!quitUserInput && !boardUserInput && std::cin.good()) { //while the user has not quit, successfully loaded a board or managed to set negative flags on the cin stream
        ...
        if(!std::cin.good()) { //if the input is raising negative flags leave loop and return negative
        ...
        else { //validate user input is either a quit or load command
            ...
            if(firstWord == COMMAND_LOAD && userVector.size() > 1) { // load
                std::string secondWord = userVector[1];
                if (Helper::isNumber(secondWord)) {
                    secondWordInt = stoi(secondWord);
                    if (secondWordInt == 1 || secondWordInt == 2) { //if load command is one of the two selectable boards, display that board
                        board->load(secondWordInt);
...
bool Game::loadBoard(std::string userInput) { // for changing boards after already selecting a board, this overload exists
    std::vector<std::string> userVector;
    int secondWordInt;
    bool boardUserInput = false;
        ...
        if(! std::cin.good()) { //if the input is raising negative flags leave loop and return negative
            ...
            if(firstWord == COMMAND_LOAD && userVector.size() > 1) { // load
                ...
                if (Helper::isNumber(secondWord)) {
                    ...
                    else { //load x was not a valid number, so print error msg, exit loop and try again
                        Helper::printInvalidInput();
                        boardUserInput = true;
                        loadBoard();
                    }
                }
                else { //load x was not a number, so print error msg, exit loop and try again
                    Helper::printInvalidInput();
                    boardUserInput = true;
                    loadBoard();
                }
            }
            else { //command was invalid, print error msg
                Helper::printInvalidInput();
            }
...
bool Game::initializePlayer() {
    ...
    while(!quitUserInput && !initUserInput && std::cin.good()) { //while user input is not quit, successful init or did not raise any negative flags for cin stream
        ...
        if(!std::cin.good()) { //if user input raised negativve flags in cin stream
            std::cout << "Something went wrong with the stream (probably ^D), exiting loop to prevent infinite loop.";
        }
        else {
            ...
            if(firstWord == COMMAND_LOAD && userVector.size() > 1) { // load
                std::string secondWord = userVector[1];
                initUserInput = true;
                loadBoard(userInput); //run loadBoard() using the desired user input
            }
                ...
                if (userVector.size() == 3) { //if init command as enough varibles (3)
                    if (Helper::isNumber(userVector[0]) && Helper::isNumber(userVector[1])) { //if the first two variables are numbers
                        ...
                        if (xPos < 10 && yPos < 10) { // error handling for x and y to make sure that they are within bounds
